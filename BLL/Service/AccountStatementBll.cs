...
{
    public class AccountStatementBll
    {
        private readonly IRepository<CalAccountChart> _repository;
        private string language = "ar";

        public AccountStatementBll(IRepository<CalAccountChart> repository)
        {
        ...
        public ResultDTO GetAccountStatementSelect(ParametersDTO parametersDTO)
        {
            ...
            var tbl = _repository.Find(p => !p.DeletedAt.HasValue && p.AccountType == 2);
            if (tbl != null)
            {
                var data = tbl.Select(p => new { Id = p.AccountId, Code = p.AccountCode, Name = parametersDTO.lang == "ar" ? p.AccountNameA : p.AccountNameE });
                resultDTO.data = data;
        ...
        public ResultDTO GetProfessorAccountStatementRpt(int accountId, string from, string to, string lang)
        {
            ...
            List<RPTAccountStatement_Result> rPTAccounts = null;
            language = lang;
            ...
            try
            {
                ...
                SqlParameter[] parameters = new[] {
                    new SqlParameter("@AccountCodeFrom", accountId) , new SqlParameter("@AccountCodeTo", accountId),
                    new SqlParameter("@TrdateFrom", fromDate),new SqlParameter("@TrdateTo", toDate)
                };

                rPTAccounts = _repository.ExecuteStoredProcedure<RPTAccountStatement_Result>("RPTAccountStatement", parameters).ToList();
                //rPTAccounts = context.RPTAccountStatement(accountId, accountId, fromDate, toDate).ToList();
            }
            catch (Exception ex)
            {
        ...
        public List<RPTAccountStatementVM> GertResult(List<RPTAccountStatement_Result> rPTAccounts)
        {
            ...
            foreach (var item in accountStatement)
            {
        ...
        #region DocType
        public string GetDocType(RPTAccountStatement_Result item)
        {
        ...
        #region LocalBalance
        ////رصيد محلى
        public decimal GetBalancLocal(List<RPTAccountStatementVM> accountStatement, RPTAccountStatement_Result rPTAccounts, decimal BalanceBeforeLocal)
        {
        ...
        public decimal GetBalanceBeforeLocal(RPTAccountStatement_Result rPTAccounts)
        {
        ...
        public decimal GetOpenBalanceLocal(RPTAccountStatement_Result rPTAccounts)
        {
